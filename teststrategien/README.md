...
| **BB-19** | Ungültig 2 Buchstaben: `aw` eingeben | Fehler "Ungültige Eingabe" + keine Aktion, erneute Eingabeaufforderung | Kein Fehler, `a` wird ausgeführt (`printAccountsList`), 2. Zeichen `w` wird ignoriert | **Fehler** | `Counter.java:40`: Regex `\d\|a\|e\|w\|q` ohne Anker + `matcher.find()` prüft nur, ob es irgendwo enthalten ist.<br>`Counter.java:49`: `substring(0,1)` wertet nur das 1. Zeichen aus. |
| **BB-20** | Buchstabe+Zahl: `a1` / `e5` eingeben | Fehler "Ungültige Eingabe" | Als `a`/`e` akzeptiert, Zahl wird ignoriert | **Fehler**  | Gleicher Fehler: `find()` findet `a` in `a1` -> switch in `Counter.java:49` |
| **BB-21** | Edit-Menu Mehrzeichen: `editAccount()` (`Counter.java:89`). Eingabe `ab` / `wq` bei `a/e/k/ü/l/w/q` | Fehler "Ungültige Eingabe" | Kein Fehler, nur 1. Zeichen geprüft | **Fehler**| `Counter.java:105`: `substring(0,1)` wird *vor* der Validierung ausgeführt.<br>`Counter.java:108`: `Pattern.compile("[aekülwq]")` + `find()` |
